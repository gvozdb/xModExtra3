xModExtra3.utils.renderBoolean = function (value) {
    return value
        ? String.format('<span class="green">{0}</span>', _('yes'))
        : String.format('<span class="red">{0}</span>', _('no'));
};

xModExtra3.utils.getMenu = function (actions, grid, selected) {
    const menu = [];
...
xModExtra3.utils.renderActions = function (value, props, row) {
    const res = [];
    ...
    for (const i in row.data.actions) {
        ...
        item = String.format(
            '<li class="{0}"><button class="xmodextra3-btn xmodextra3-btn-default {1}" action="{2}" title="{3}"></button></li>',
    ...
    return String.format(
        '<ul class="xmodextra3-row-actions">{0}</ul>',
        res.join('')
    );
};